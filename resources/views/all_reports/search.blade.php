	...
	<div class="col-md-12">
		<div class="card">
			...
			<div class="card-body">
				...
				<form method="POST" name="Search" action="{{ route('search') }}">
					{{ method_field('POST') }}
					{{ csrf_field() }}
					<div class="row m-b-15">
							...
							<div class="row">
								<div class="col-xs-8 col-sm-8 col-md-8">
									<select name="user" class="form-control">
										...
										<option value="{{ $user -> id }}" {{ ($user -> id == $selectedUser) ? 'selected' :'' }}>{{ $user -> full_name }}</option>
										@endforeach
									</select>
								</div>
						...
						{{-- <div class="col-xs-10 col-sm-10 col-md-10">
							<input type="text" name="q" class="form-control" placeholder="Search reports by user name..">
						</div> --}}
						{{-- <div class="col-xs-2 col-sm-2 col-md-2">
							<button type="submit" class="btn btn-default btn-block"><i class="fa fa-search"></i> Search Reports</button>
						</div> --}}
					</div>
				</form>
